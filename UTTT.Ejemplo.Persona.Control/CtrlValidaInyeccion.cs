 ...
 public  class CtrlValidaInyeccion
    {
        ...
        public bool InyectionFecha(string _informacion, ref string _mensaje, string _etiquetaReferente,
           ref System.Web.UI.WebControls.TextBox _control)
        {
            
            Regex tagRegex = new Regex(@"^[a-zA-Z-ZÀ-ÿ\u00f1\u00d1\s]+$");
            bool respuesta = tagRegex.IsMatch(_informacion);
            if (respuesta)
            {
                _mensaje = "Caracteres inválidos en " + _etiquetaReferente.Replace(":", "");
                _control.Focus();
            }
            return respuesta;
